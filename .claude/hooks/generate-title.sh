...
if [ "$session_exists" -gt 0 ]; then
    ...
    # Skip title/topic update if titleFixed is true (user pinned the title)
    updated=$(echo "$existing" | jq \
        ...
        .tasks = [.tasks[] | if .sessionId == $sid then
            .status = $status |
            .updatedAt = $ts |
            (if $title != "" and (.titleFixed != true) then .title = $title else . end) |
            (if $topic != "" and (.titleFixed != true) then .topic = $topic else . end) |
