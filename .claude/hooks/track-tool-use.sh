#!/bin/bash
# Hook: PostToolUse - Called after a tool has been used (permission granted)
...
# Update task status to working (permission was granted, back to work)
updated=$(echo "$existing" | jq --arg sid "$SESSION_ID" --arg status "working" --arg ts "$timestamp" '
    .tasks = [.tasks[] | if .sessionId == $sid then .status = $status | .updatedAt = $ts | del(.pendingTool) else . end]
')
